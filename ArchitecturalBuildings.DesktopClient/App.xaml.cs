...
namespace ArchitecturalBuildings.DesktopClient
{
    ...
    public partial class App : Application
    {
        ...
        private void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddSingleton<CachedReadOnlyArcBuildingsRepository>(
                x => new CachedReadOnlyArcBuildingsRepository(
                    x.GetRequiredService<NetworkArcBuildingsRepository>(),
                    x.GetRequiredService<IDomainObjectsCache<ArcBuildings>>()
                )
            );
            services.AddSingleton<IReadOnlyArcBuildingsRepository>(x => x.GetRequiredService<CachedReadOnlyArcBuildingsRepository>());
